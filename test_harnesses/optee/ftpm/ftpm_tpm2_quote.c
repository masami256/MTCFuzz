...
typedef struct {
    ...
    union {
        ...
        struct { int dummy; } invsess; /* future fields */
    } u;
} ParsedInput;
                                     ...
                                     char *err, size_t errsz)
{
    ...
    char *c1 = line, *c2 = NULL, *c3 = NULL, *p = line;
    char *comma = strchr(p, ',');
    ...
    if (strcmp(c1, "qualifyingData") == 0) {
        if (!c2 || !c3) {
            snprintf(err, errsz, "qualifyingData requires 3 columns");
            return -1;
        }
        ...
        out->u.qd.buf_len = parse_byte_list(c3, out->u.qd.buf, QD_BUF_MAX);
        return 0;
    }
...
/* Build an INVALID sessions packet to test tag/auth inconsistency.
   Variant: tag=NO_SESSIONS but includes an auth area. */
                                     ...
                                     uint32_t ak_handle)
{
    ...
    /* header: NO_SESSIONS (but we will add auth area anyway) */
    *(uint16_t *) p = htobe16(TPM2_ST_NO_SESSIONS);
    p += 2;
    uint32_t *size_ptr = (uint32_t *) p;
    p += 4;
    *(uint32_t *) p = htobe32(TPM2_CC_Quote);
...
int ftpm_tpm2_quote_start_fuzz_test(options_t *fuzz_opt)
{
    ...
    case TEST_QD_ONLY:
        reqlen = build_qd_only(req, sizeof(req), ak,
                               pi.u.qd.size, pi.u.qd.buf, pi.u.qd.buf_len);
        if (!reqlen) {
            EPRINTF("build_qd_only failed\n");
            return -1;
        }
        IPRINTF("[qd]Send qd.size=0x%04x  buffer_len=%zu\n",
    ...
    case TEST_INVALID_SESSIONS:
        ...
        if (!reqlen) {
            EPRINTF("build_invalid_sessions failed\n");
            return -1;
        }
        IPRINTF("[invalid_sessions]Send packet\n");
        break;

    default:
        EPRINTF("unknown kind\n");
        return -1;
    }

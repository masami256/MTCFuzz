...
result_dir="../test_result_ftpm_fuzz_nvwrite/${check_type}_results"
...
for i in $(seq 1 $num_loop); do
    ...
    ../report/coverage-report.py --output "${result_dir}/$test_$(printf '%05d' "$i")_cover-${check_type}.csv" \
        --config-json "../configs/optee/ftpm_fuzz/ftpm_nv_write_${check_type}_config.json" --check-firmware-coverage --check-kernel-coverage
done

...
result_dir="../test_result_xtest_fuzz_1000/${check_type}_results"
...
for i in $(seq 1 $num_loop); do
    ...
    ../report/coverage-report.py --output "${result_dir}/$test_$(printf '%05d' "$i")_cover-${check_type}.csv" \
        --config-json "../configs/optee/xtest_fuzz_1000/optee_xtest_fuzz_1000_${check_type}_config.json" --check-firmware-coverage --check-kernel-coverage
done

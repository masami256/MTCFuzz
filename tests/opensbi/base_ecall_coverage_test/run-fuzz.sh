...
result_dir="../test_result_base_ecall_cover_test/${check_type}_results"
...
for i in $(seq 1 $num_loop); do
    ...
    ../report/coverage-report.py --output "${result_dir}/$test_$(printf '%05d' "$i")_cover-${check_type}.csv" \
        --config-json "../configs/opensbi/coverage_test/coverage_test_${check_type}_check_config.json" --check-firmware-coverage --check-kernel-coverage
done
